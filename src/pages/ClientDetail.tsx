...
export default function ClientDetail() {
  ...
  return (
    <div className="p-8 max-w-4xl">
      <button onClick={() => navigate('/clients')} className="flex items-center gap-1 text-apple-blue text-[14px] font-medium mb-6 hover:opacity-70">
        <ChevronLeft size={17} /> Clients
      </button>

      {/* Profile header */}
      <div className="apple-card p-6 mb-6 flex items-start gap-5">
        <div className={cn('w-16 h-16 rounded-full flex items-center justify-center text-white text-[20px] font-bold shrink-0', getAvatarColor(investor.name))}>
          {getInitials(investor.name)}
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-[24px] font-bold text-apple-label tracking-tight">{investor.name}</h1>
        ...
        <div className="flex gap-2 shrink-0">
          {isAdmin && (
            <button onClick={() => setEditOpen(true)} className="apple-btn-secondary flex items-center gap-2 text-[14px]">
              <Edit2 size={14} /> Edit
            </button>
          )}
          <button onClick={() => setTxOpen(true)} className="apple-btn-primary flex items-center gap-2 text-[14px]">
            <Plus size={14} strokeWidth={2.5} /> Transaction
          </button>
        </div>
      </div>
      ...
      <div className="apple-card overflow-hidden">
        ...
        {/* Table header */}
        <div className={cn('grid gap-4 px-6 py-2 bg-apple-surface-2', isAdmin ? 'grid-cols-[auto_1fr_auto_auto_auto]' : 'grid-cols-[auto_1fr_auto_auto]')}>
            ...
            {displayTxs.map((tx: Transaction & { running: number }, idx) => {
              ...
              return (
                <div
                  key={tx.id}
                  className={cn(
                    'grid gap-4 items-center px-6 py-4 group',
        ...
        {/* Totals footer */}
        <div className="bg-apple-surface-2 border-t border-apple-sep-dark px-6 py-4">
          <div className={cn('grid gap-4', isAdmin ? 'grid-cols-[auto_1fr_auto_auto_auto]' : 'grid-cols-[auto_1fr_auto_auto]')}>
            <div className="w-24" />
